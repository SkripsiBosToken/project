<x-layout.admin-v2>
    <form action="{{ route('system.customer.update') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="content my-3">
            <div class="d-flex flex-wrap gap-3">
                @foreach ($customers as $key => $item)
                    <div class="card">
                        <div class="card-header"><strong class="card-title">Pelanggan {{ $key + 1 }}</strong></div>
                        <div class="card-body">
            ...
            <button type="button" class="btn btn-success mt-3" onclick="addCustomerCard()">
                <i class="fa fa-plus-circle mr-1" aria-hidden="true"></i> Tambah Pelanggan
            </button>
    ...
    <template id="customer-template">
        <div class="card">
            <div class="card-header"><strong class="card-title">Pelanggan Baru</strong></div>
            <div class="card-body">
                <div class="form-group">
                    <label>Nama Customer</label>
...
<script>
    let customerIndex = {{ count($customers) }};
    let deletedIndexes = [];
